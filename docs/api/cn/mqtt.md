...
上报主题在北向节点订阅中指定，其默认值为 **/gridbeat/{MQTT driver name}** ，见下图
...
通过自定义数据格式，添加全局静态点位。该静态点位在所有驱动采集组上报数据时，均会携带。如 gridbeatEX 部署在一个物理网关上，可将网关的SN号、IP地址、位置信息等信息，添加到所有驱动采集组中。
...
如果添加的静态点位，为 gridbeatEX 实例中所有采集组共用，既可以勾选**所有采集组**，添加静态点位，也可以在 Custom 自定义格式中，统一添加静态点位，参考 [自定义格式：示例二](#自定义格式)。
...
通过发送 JSON 形式的请求到 MQTT 主题 **/gridbeat/{node_name}/read/req**，您可以读取一组点位数据。
...
读响应会发布到 MQTT 主题 **/gridbeat/{node_name}/read/resp** 。
...
通过发送 JSON 形式的请求到**写请求主题**参数指定的 MQTT 主题，您可以写一个点位数据。可在 MQTT 参数配置中配置写请求的主题，默认为为 **/gridbeat/{random_str}/write/req**。
...
gridbeat 提供了对多点位写入的支持。要在一次请求中可以写入多个点位数据, 写请求体应由以下字段构成：
* `uuid` : 唯一的标志，会在响应中原样返回用以区分对应的请求。
* `node` : 某个南向节点名字。
* `group` : 南向节点的某个组的名字。
* `tags` : 点位数据数组，每个元素对应一个点位。
...
写响应会发布到**写响应主题**参数指定的 MQTT 主题。默认**写响应主题**参数为 **/gridbeat/{random_str}//write/resp**。
...
上报主题在北向节点配置中指定，其默认值为 **/gridbeat/{random_str}/state/update** 
